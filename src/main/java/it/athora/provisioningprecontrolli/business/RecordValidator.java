...
public final class RecordValidator {
	...
	public static RecordValidator from(AgenziaListino agenziaListino) {
		List<LogPredicate> checks = Arrays.asList(
				new LogPredicate(record -> !record.isMalformed(),
						record -> "Record '" + record.getOriginalLine() + "' formato non valido"),

				new LogPredicate(record -> TextUtils.allNotBlank(record.getMandatoryFieldsAsList()),
				...
				new LogPredicate(record -> !NON_ASCII_PATTERN.matcher(record.getOriginalLine()).find(),
						record -> "Record '" + record.getOriginalLine() + "' contiene caratteri non-ASCII"),

				new LogPredicate(record -> EMAIL_PATTERN.matcher(record.getEmail()).matches(),
						record -> "Record '" + record.getOriginalLine() + "' email non valida"),

				new LogPredicate(record -> agenziaListino.isAgenziaValid(record.getAgenzia()),
						record -> "Record '" + record.getOriginalLine() + "' agenzia non valida"),

				new LogPredicate(record -> agenziaListino.isListinoValid(record.getListino()),
				...
				new LogPredicate(record -> {
					if (TextUtils.isBlank(record.getDataFine()))
						return true;
					try {
